    ...
    <div className="bg-surface text-on-surface antialiased">
      ...
      <section className="py-24 bg-surface-bright">
        <div className="px-6 md:px-12 mb-12">
          <h2 className="text-3xl tracking-wide text-primary font-serif">
        ...
        <div className="flex gap-8 overflow-x-auto px-6 md:px-12 no-scrollbar pb-8">
          {SAMPLE_STONES.map((stone) => (
            <button
              ...
              className="flex-none w-48 group text-left cursor-pointer"
            >
      ...
      <section className="py-24 bg-[#fcf9f4]">
        <div className="px-6 md:px-12 flex justify-between items-end mb-16">
          <div>
            <span className="font-label uppercase tracking-widest text-[#8f4c30] text-xs font-bold">
              Arriving Weekly
            </span>
            <h2 className="text-4xl text-[#31032c] mt-2 font-serif font-light">
              New Treasures
            </h2>
          </div>
          ...
          <Link
            to="/shop"
            className="text-[#4f434b] font-label text-xs uppercase tracking-widest hover:text-[#31032c] transition-colors font-bold border-b pb-1 border-[#31032c]/20"
          >
            Shop All
          </Link>
        </div>
        ...
        <div className="px-6 md:px-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {featured.map((p) => (
            <div
              key={p.id}
              className="group flex flex-col h-full bg-white rounded-none p-5 transition-all duration-500 hover:shadow-xl relative border border-[#31032c]/10"
            >
              ...
              <div className="flex-grow flex flex-col justify-between">
                <div>
                  <h3 className="font-label text-sm uppercase tracking-wider text-[#31032c] font-bold mb-1">
                    {p.name}
                  </h3>
                  <p className="text-[#4f434b]/80 text-sm italic font-serif mb-4">
                    {p.stoneColor} & 18k Gold
                  </p>
                </div>

                <div className="flex justify-between items-center mt-2">
                  <span className="text-[#31032c] font-medium tracking-wide">
                    ${p.price.toLocaleString()}
                  </span>

                  <Link
                    to={`/shop/${p.id}`}
                    className="text-[#8f4c30] text-xs uppercase tracking-widest font-bold hover:underline underline-offset-4 font-sans"
                  >
                    Quick Buy
                  </Link>
                </div>
      ...
      <section className="py-24 bg-[#f0ede9]/30 artistic-dotted-bg px-6 md:px-12 lg:px-24 border-t border-[#31032c]/10">
        <div className="flex flex-col items-center text-center mb-20">
          <h2 className="text-4xl md:text-5xl text-[#31032c] mb-6 font-serif">
            The Atélier Journal
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          {SAMPLE_BLOGS.map((blog) => (
            <div key={blog.id} className="group cursor-pointer">
              ...
              <h3 className="text-[2rem] text-[#31032c] font-serif italic mb-4 group-hover:text-[#8f4c30] transition-colors">
                {blog.title}
              </h3>

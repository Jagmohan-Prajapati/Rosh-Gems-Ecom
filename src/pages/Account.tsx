...
export const Account: React.FC = () => {
  ...
  const [activeSubTab, setActiveSubTab] = useState<ActiveTab>("profile");
  ...
  const fetchMyOrders = async () => {
    try {
      ...
      const d = await res.json();
      setMyOrders(Array.isArray(d.orders) ? d.orders : []);
    } catch (e) {
      console.error(e);
    }
  };
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMessage(
        e instanceof Error ? e.message : "Communications failure with vault backend."
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(await getErrorMessage(res, "Incorrect current password."));
      }
      ...
      setErrorMessage(
        e instanceof Error ? e.message : "Failed to connect to the password workbench."
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleAddressSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(await getErrorMessage(res, "Failed to save shipment credentials."));
      }
      ...
      setErrorMessage(
        e instanceof Error ? e.message : "Could not connect to address dispatcher."
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleDeleteAddress = async (id: string) => {
    ...
    if (!window.confirm("Delete this shipment address permanently from the vault?")) {
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(await getErrorMessage(res, "Failed to delete address."));
      }

      await fetchAddresses();
      setSuccessMsg("Address unsealed and discarded.");
    } catch (e) {
      setErrorMessage(e instanceof Error ? e.message : "Failed to delete address.");
    }
  };

  const orderCountLabel = useMemo(() => myOrders.length, [myOrders.length]);

  return (
    <div className="pt-32 pb-24 px-6 md:px-12 max-w-7xl mx-auto min-h-screen">
      <header className="mb-12">
        <span className="text-[10px] uppercase tracking-[0.4em] text-[#D4AF37] block mb-4 font-semibold">
          PATRON CONTROL BOARD
        </span>
        ...
        <p className="text-[10px] tracking-widest opacity-40 uppercase mt-1 font-body">
          Account rank: {user?.role || "Patron"}
        </p>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-12 items-start">
        <div className="bg-[#121412] border border-[#D4AF37]/15 p-6 rounded-xl space-y-2">
          ...
          >
            ...
            <span>Acquisition Logs ({orderCountLabel})</span>
          </button>
        </div>
        ...
        <div className="lg:col-span-3">
          {activeSubTab === "profile" && (
            <div className="space-y-12">
              <div className="bg-[#121412]/50 border border-[#D4AF37]/10 rounded-xl p-8 space-y-6">
                <h3 className="font-headline text-2xl italic text-[#D4AF37]">
                ...
                <form onSubmit={handleUpdateProfile} className="space-y-6 max-w-xl">
                  ...
                  <div>
                    <span className="text-[9px] uppercase tracking-widest text-[#F5F5F0]/40 font-semibold block">
                      Registered Email
                    </span>
                  ...
                  <button
                    type="submit"
                    disabled={loading}
                    className="px-8 py-3 bg-[#D4AF37] text-black font-semibold text-xs tracking-widest uppercase hover:bg-[#B8962F] transition-all disabled:opacity-70 disabled:cursor-not-allowed"
                  >
              ...
              <div className="bg-[#121412]/50 border border-[#D4AF37]/10 rounded-xl p-8 space-y-6">
                ...
                <form onSubmit={handleChangePassword} className="space-y-6 max-w-xl">
                  ...
                  <button
                    type="submit"
                    disabled={loading}
                    className="px-8 py-3 bg-[#121412] border border-[#D4AF37] hover:bg-[#D4AF37]/10 text-[#D4AF37] font-semibold text-xs tracking-widest uppercase transition-all disabled:opacity-70 disabled:cursor-not-allowed"
                  >
                    {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : "Modify Entry Key"}
                  </button>
                </form>
              </div>
            </div>
            ...
            <div className="space-y-8 animate-fade-in">
              ...
              {showAddressForm && (
                <form
                  ...
                  className="bg-[#121412]/60 p-6 border border-[#D4AF37]/20 rounded-xl space-y-6 animate-fade-in"
                >
                  <h4 className="font-headline text-lg italic text-[#D4AF37]">
                    {editAddressId ? "Edit Shipment Address" : "Register Shipment Address"}
                  </h4>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="py-2 text-xs tracking-wider border-b border-[#D4AF37]/10 text-white select-none">
                        India
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-4 items-center pt-2">
                    <button
                      type="submit"
                      disabled={loading}
                      className="px-6 py-2.5 bg-[#D4AF37] text-black font-semibold text-xs tracking-widest uppercase hover:bg-[#B8962F] transition-all disabled:opacity-70 disabled:cursor-not-allowed"
                    >
                      {loading ? (
              ...
              {!showAddressForm && addresses.length === 0 && (
                <div className="text-center py-16 bg-[#121412]/50 border border-[#D4AF37]/10 rounded-xl">
                  ...
                  <p className="text-sm text-[#F5F5F0]/60">
                    Your physical shipment vault contains zero addresses
                  </p>
                </div>
              )}
              ...
              {!showAddressForm && addresses.length > 0 && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {addresses.map((addr) => (
                    <div
                      key={addr.id}
                      className="p-5 bg-[#121412]/60 border border-[#D4AF37]/10 hover:border-[#D4AF37]/35 rounded-lg flex flex-col justify-between space-y-4"
                    >
                      <div>
                      ...
                      <div className="flex items-center gap-3 pt-4 border-t border-[#D4AF37]/10 text-[10px] font-bold tracking-widest uppercase">
                        <button
                          ...
                          className="flex items-center gap-1 text-[#D4AF37] hover:underline bg-transparent border-none"
                        >
          ...
          {activeSubTab === "orders" && (
            <div className="space-y-6 animate-fade-in">
              <h3 className="font-headline text-2xl italic text-[#D4AF37] mb-6">
                Historical Acquisitions Logs
              </h3>
              ...
              {myOrders.length === 0 ? (
                <div className="text-center py-20 bg-[#121412]/50 border border-[#D4AF37]/10 rounded-xl">
                  ...
                  <p className="text-sm text-[#F5F5F0]/60">
                    Your historical ledger accounts for zero transactions
                  </p>
                  <Link
                    to="/shop"
                    className="text-xs text-[#D4AF37] underline underline-offset-4 mt-3 block font-semibold hover:text-[#B8962F]"
                  >
                    Source Your First Gemstone
                  </Link>
                </div>
              ) : (
                    ...
                    >
                      <div className="space-y-1">
                        <div className="flex items-center gap-3">
                          ...
                          <span
                            className={`px-2 py-0.5 rounded-[3px] text-[8px] tracking-widest font-bold uppercase ${
                              ord.isPaid
                      ...
                      <div className="pt-2 md:pt-0">
                        <span className="opacity-40 text-[9px] uppercase tracking-widest block mb-0.5">
                          Carriage Route Status
                        </span>
                      ...
                      <div className="text-left md:text-right">
                        <span className="opacity-40 text-[9px] uppercase tracking-widest block mb-0.5 animate-pulse">
                          Total Settled
                        </span>
                        <span className="text-sm font-headline text-[#D4AF37] font-semibold">
